...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = IdentityEncoder(obs_shape, encoder_feature_dim, num_layers, num_filters)
...
class CURL(nn.Module):
    ...
    def __init__(
        self, obs_shape, z_dim, batch_size, critic, critic_target, output_type="continuous"
    ):
...
class SacAgent(object):
    ...
    def select_and_sample_action(self, obs):
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(self.device)
            obs = obs.unsqueeze(0)
            mu, pi, _, _ = self.actor(obs, compute_pi=True)
            return mu.cpu().data.numpy().flatten(), pi.cpu().data.numpy().flatten()

    def select_action(self, obs):
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(self.device)
            obs = obs.unsqueeze(0)
            mu, _, _, _ = self.actor(obs, compute_pi=True, compute_log_pi=False)
    ...
    def save(self, model_dir, step):
        ...
        torch.save(self.actor.state_dict(), f"{model_dir}critic_{step}.pt")
...
class EnsembleSacAgent(nn.Module):
    """ Wrapper of `CurlSacAgent` to work with rest of codebase """

    def __init__(self, sac_agents, buffer, stochastic=False, update_sac=True):
        super().__init__()
    ...
    def save(self, logdir):
        for i, agent in enumerate(self.sac_agents):
            agent.save(logdir, i)
    ...
    def forward(self, state):
        ...
        for agent in self.sac_agents:
            with utils.eval_mode(agent):
                ...
                mu, pi  = agent.select_and_sample_action(state)
                mus.append(mu)
                pis.append(pi)

        """ TODO how to select from ensemble of SAC? (can also get std of output) """

        agent_idx = np.random.randint(0, len(self.sac_agents))
        if self.stochastic:
            action = mus[agent_idx]
        else:
            action = pis[agent_idx]
